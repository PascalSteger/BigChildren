...
function usage() {
  echo "Tool for sending data to the BigChildren application"
  echo "Usage: $script [--host <hostname>]"
  echo ""
  echo "  Options"
  echo "    --host      Specifies the host that CDAP is running on. (Default: localhost)"
  echo "    --help      This help message"
  echo "    --country   specify two chars representing country"
  echo ""
}
...
echo "sending child mortality data"
stream="childmortalityStream"
OLD_IFS=IFS
IFS=$'\n'
lines=`cat "$bin"/../resources/$country/childmortality.txt`
for line in $lines
do
...
echo "sending weather data"
stream="weatherStream"
OLD_IFS=IFS
IFS=$'\n'
lines=`cat "$bin"/../resources/$country/$station/alldata`
for line in $lines
do
  status=`curl -qSfsw "%{http_code}\\n" -H "Authorization: Bearer $auth_token" -X POST -d "$line" http://$gateway:10000/v2/streams/$stream`
  if [ $status -ne 200 ]; then
