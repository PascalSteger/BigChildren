...
function usage() {
  echo "Application lifecycle management tool for the BigChildren application."
  echo "Usage: $script --action <deploy|start|run|stop|status> [--host <hostname>] [--country <country>] [--station <station>]"
  echo ""
  echo "  Options"
  ...
  echo "    --country   string of 2 chars for country to look at"
  echo "    --station   6-digit identifier for station to look at"
...
do
    case "$1" in
        ...
        --country) shift; country="$1"; shift;;
        --station) shift; station="$1"; shift;;
        *)  usage; exit 1
    esac
done
...
if [ "x$action" == "xdeploy" ]; then
    jar_path=`ls $dir/../target/BigChildren-*.jar`
    deploy_action $app $jar_path $host
elif [ "x$action" == "xrun" ]; then
    program_action $app "BigChildrenProgram" "spark" "start" $host $country $station
else
    program_action $app "WeatherFlow" "flow" $action $host
    program_action $app "ChildMortalityFlow" "flow" $action $host
    program_action $app "CorrelationsProcedure" "procedure" $action $host
    #  dont start and stop mapreduce if start/stop is specified
    if [ "x$action" == "xstatus" ]; then
        program_action $app "BigChildrenProgram" "spark" $action $host
    fi
fi
